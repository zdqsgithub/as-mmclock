#!/usr/bin/env python3
"""Run the fixed v7.1 five-dataset harmonization smoke grid."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--matrix",
        default=str(ROOT / "results" / "multidataset_v7_1" / "all_rrbs_region_matrix_5kb.parquet"),
    )
    parser.add_argument(
        "--metadata_path",
        default=str(ROOT / "metadata" / "model_sample_metadata_v7_1.csv"),
    )
    parser.add_argument("--out_root", default=str(ROOT / "results" / "benchmark_v7_1_harmonization"))
    ...
    summary.to_csv(out_root / "v7_1_smoke_summary.tsv", sep="\t", index=False)
